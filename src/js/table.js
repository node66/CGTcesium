...
const [tableBody] = tableObject.getElementsByTagName('tbody');
let id = 0;
export {
  id,
};
...
export function addRow(filename) {
  ...
  checkboxCell.innerHTML = `<input checked class="form-check-input" id="check${id}" name="checkbox" type="checkbox" value="">`;
  ...
  imgCell.innerHTML = `<label for="pngFile${id}" class="form-label">
    ...
    if (file) {
      ...
      data.entities.values.forEach(entity => {
        ...
        entity.billboard = new Cesium.BillboardGraphics({
          image: path, width: new Cesium.CallbackProperty(() => {
            return Number(sizeInp.value);
          }, false), height: new Cesium.CallbackProperty(() => {
            return Number(sizeInp.value);
          }, false),
        });
      });
  ...
  polyCheck.addEventListener('change', () => {
    ...
    const [data] = viewer.dataSources.getByName(nameCell.innerHTML);
    data.entities.values.forEach(entity => {
      cartesianCoord.push(entity.position.getValue(Cesium.JulianDate.now()));
    });
    const polyline = viewer.entities.getById(nameCell.innerHTML)?.polyline;
    if (polyline === undefined) viewer.entities.add({
      id: nameCell.innerHTML, polyline: {
        positions: cartesianCoord, arcType: Cesium.ArcType.NONE,
        width: 2, //may be need to
    ...
    }); else polyline.show = !!(polyCheck.checked && checkBox.checked);
  });
  ...
  colorCell.innerHTML =
      ...
      '   <td colspan="2"><label for="colorInput${id}" class="form-label"></label>' +
      `      <input type="color" class="form-control form-control-color" id="colorInput${id}" value="#FFFFFF">` +
      '</td>' +
      ' </tr>' +
      ' <tr>' +
      '   <th scope="row">3d buffer</th>' +
      '   <td><label for="colorInput${id}" class="form-label"></label>' +
      `      <input type="color" class="form-control form-control-color" id="colorInput${id}" value="#FFFFFF">` +
      '</td>' +
      ' </tr>' +
      '</table>';
  ...
  function sizeAndColorChange() {
    const [data] = viewer.dataSources.getByName(nameCell.innerHTML);
    const showPoint = data.entities.values[0].billboard === undefined;
    data.entities.values.forEach(entity => {
      if (showPoint) {
        entity.point = {
          pixelSize: new Cesium.CallbackProperty(() => {
            return sizeInp.value;
          }, false), color: new Cesium.CallbackProperty(() => {
            return Cesium.Color.fromCssColorString(colorInp.value);
          }, false), show: true,
        };
      } else if (!showPoint) {
  ...
  sizeCell.innerHTML = `<input id="sizeInput${id}" type="number" min="0" required value="10">`;
